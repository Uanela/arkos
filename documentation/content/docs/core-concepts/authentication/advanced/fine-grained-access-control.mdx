...
`ArkosPolicy` works identically in both [Static](/docs/core-concepts/authentication/permissions/static) and [Dynamic](/docs/core-concepts/authentication/permissions/dynamic) mode. The only difference is that `roles` inside rules are enforced in Static mode and ignored in Dynamic mode — enforcement comes from the database instead. In Dynamic mode, define your rules without roles:
```ts title="src/modules/order/order.policy.ts"
import { ArkosPolicy } from "arkos";
