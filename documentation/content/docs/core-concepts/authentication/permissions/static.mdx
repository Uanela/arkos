...
Permissions wire the same way whether you're on a custom route or a built-in one. For built-in routes (Prisma model, auth, file upload) use a [Route Hook](/docs/core-concepts/components/route-hooks) alongside your `ArkosRouter` export.
...
The same auth config file works on both custom and built-in routes. For built-in routes use a [Route Hook](/docs/core-concepts/components/route-hooks) alongside your `ArkosRouter` export.
