...
Same `ArkosPolicy` and `.auth.ts` API as [Static mode](/docs/core-concepts/authentication/modes/static/setup) — the only difference is that `roles` inside rules are ignored. Actual enforcement comes from database records instead. Define your policy with names and descriptions for discovery, skip the roles:
...
Wiring to routes is identical to Static mode — see [Static Mode — Wiring to Routes](/docs/core-concepts/authentication/modes/static/setup#wiring-to-routes).

<Callout type="info">
To protect [authentication routes](/docs/core-concepts/authentication/customization/interceptors) or [file upload routes](/docs/guides/file-upload/authentication) see their respective guides.
</Callout>
