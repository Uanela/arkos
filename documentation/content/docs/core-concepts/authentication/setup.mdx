...
Arkos provides a JWT-based authentication and authorization system with Role-Based Access Control (RBAC) that secures your auto-generated Prisma model routes, built-in auth routes, file upload routes, and custom [ArkosRouter](/docs/core-concepts/components/arkos-router) routes out of the box. This page covers the general authentication setup shared across all permission modes and helps you understand how Arkos handles permissions.

## Permission Modes

Once a user is authenticated, Arkos needs to know what they're allowed to do. It handles this through two permission modes — [Static](/docs/core-concepts/authentication/modes/static/setup) and [Dynamic](/docs/core-concepts/authentication/modes/dynamic/setup).

### Static

[Static mode](/docs/core-concepts/authentication/modes/static/setup) defines permissions in code via [`ArkosPolicy`](/docs/core-concepts/authentication/modes/static/setup#arkospolicy) (v1.6+) or [`.auth.ts` files](/docs/core-concepts/authentication/modes/static/setup#auth-files). Roles are assigned directly on the `User` model as an enum field. Use Static when your roles are stable and known at deploy time — most apps start here.

### Dynamic

[Dynamic mode](/docs/core-concepts/authentication/modes/dynamic/setup) stores permissions in the database via `AuthRole`, `AuthPermission`, and `UserRole` models. Roles and permissions can be created, updated, and assigned at runtime without a redeploy. Use Dynamic when permissions need to be managed at runtime — multi-tenant apps, SaaS platforms, or any system where roles change frequently.

| | [Static](/docs/core-concepts/authentication/modes/static/setup) | [Dynamic](/docs/core-concepts/authentication/modes/dynamic/setup) |
|---|---|---|
...
<Callout type="info">
`role` / `roles` is for [Static mode](/docs/core-concepts/authentication/modes/static/setup) only. [Dynamic mode](/docs/core-concepts/authentication/modes/dynamic/setup) replaces it with database-driven role relations.
</Callout>

## Protecting Routes

Before diving into permission modes, any route can be protected with `authentication: true` — this simply requires the user to be logged in, regardless of their role or permissions. For role-based access control, see [Static Mode](/docs/core-concepts/authentication/modes/static/setup) or [Dynamic Mode](/docs/core-concepts/authentication/modes/dynamic/setup).
...
→ [Static Mode](/docs/core-concepts/authentication/modes/static/setup) — define permissions in code via `ArkosPolicy` or `.auth.ts` files

→ [Dynamic Mode](/docs/core-concepts/authentication/modes/dynamic/setup) — manage permissions at runtime via database
