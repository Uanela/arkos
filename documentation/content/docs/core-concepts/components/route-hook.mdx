...
Route Hook is an Arkos component that lets you configure auto-generated routes without touching the route definitions themselves. It is a named export from your module's `*.router.ts` file where each key maps to a built-in operation, and each value is the same configuration object accepted by [ArkosRouter](/docs/core-concepts/components/arkos-router).
...
export const hook: RouteHook = {
  ...
  createOne: { authentication: { resource: "post", action: "Create", rule: ["admin"] } },
...
Every key in a Route Hook accepts the same configuration object as [ArkosRouter](/docs/core-concepts/components/arkos-router) routes — authentication, validation, rate limiting, disabled, and more. See the full reference at [ArkosRouter Configuration Object](/docs/reference/arkos-router#configuration-object).
...
```
DeprecationWarning: `export const config: RouterConfig` in post.router.ts is deprecated.
