...
A Gateway is created with `ArkosGateway(config)` and wired into a `socket.io` `Server` with `.register(io)`. Everything else — defining events, socket enhancements, pipes, hooks, deduplication — lives under [WebSockets](/docs/guides/websockets/setup).
