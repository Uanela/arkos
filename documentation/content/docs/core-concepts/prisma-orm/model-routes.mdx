...
Every generated endpoint accepts the same configuration object used in `ArkosRouter` — disable routes, add rate limiting, authentication, and more via the `config` export in your router file `src/modules/<model>/<model>.router.ts` where each key maps to the operation name from the table above:
...
See [Model Interceptors](/docs/core-concepts/prisma-orm/interceptors) for the full list of available hooks and what you can do with them.
