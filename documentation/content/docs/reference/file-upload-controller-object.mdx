...
All controller methods use Arkos's `catchAsync` wrapper, which automatically handles errors and passes them to the error handling middleware. For more information about error handling, see the [catchAsync function documentation](/docs/reference/the-catch-async-function).
