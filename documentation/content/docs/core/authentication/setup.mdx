...
Before diving into permission modes, any route can be protected with `authentication: true` in your `ArkosRouter` config — this simply requires the user to be logged in, regardless of their role or permissions:
...
### Protecting Built-in Routes

For auto-generated Prisma model routes, use the `config` export in your `.router.ts` file:
```ts title="src/modules/post/post.router.ts"
...
See [Prisma Model Routes](/docs/core-concepts/prisma-orm/model-routes#authentication), [Authentication Routes](/docs/core-concepts/authentication/customization/interceptors), and [File Upload Routes](/docs/guides/file-upload/authentication) for full configuration options.
