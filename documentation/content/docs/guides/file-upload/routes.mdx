...
<Callout type="info">
  `/api/uploads` is the default base route and can be customized in your config. See [File Upload Setup](/docs/guides/file-uploads/setup) for details.
</Callout>

## Two Approaches to File Uploads

These standalone routes are one way to handle file uploads. Arkos also supports declaring uploads directly on your model routes via `RouterConfig` — a single API call that handles both the file and the record together.
...
Use the `config` named export in `src/modules/file-upload/file-upload.router.ts` to configure any file upload endpoint. Each key maps to the operation name from the table above:
...
export const config: RouterConfig = {
  findFilie: { authentication: false }, // public
...
Available hooks: `beforeUploadFile`, `afterUploadFile`, `beforeUpdateFile`, `afterUpdateFile`, `beforeDeleteFile`, `afterDeleteFile`, `beforeFindFile`. See [File Upload Interceptors](/docs/guides/file-upload/interceptors) for full details.
