...
`_meta.mid` and `_meta.timestamp` are injected on every emit automatically — this is what makes [deduplication and freshness checks](/docs/guides/websockets/advanced/deduplication-and-freshness) work without you touching them.
...
Framework bindings wrap this in your framework's lifecycle/reactivity primitives so you don't manage `off()`/`unsub()` calls by hand — see [React](/docs/guides/websockets/advanced/frontend-integrations/react) for the shipped example.
