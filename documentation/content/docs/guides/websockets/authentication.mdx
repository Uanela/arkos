...
A child Gateway registered via `.use()` inherits its parent's `authentication` setting unless it explicitly overrides it. See [Setup → Attaching connection middleware or nesting Gateways](/docs/core-concepts/websockets/setup#attaching-connection-middleware-or-nesting-gateways--use).
...
Authorization runs after rate limiting and before validation. See the full ordering in [Handling Events → Full request lifecycle for one event](/docs/core-concepts/websockets/handling-events#full-request-lifecycle-for-one-event).
