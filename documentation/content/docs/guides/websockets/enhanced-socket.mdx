...
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `socket.currentUser` | After successful auth, if the Gateway has `authentication: true`. See [Authentication](/docs/guides/websockets/authentication). |
| `socket.data`        | Incoming data, if the event has a `validation` schema it is the validated and transformed data. See [Validation](/docs/guides/websockets/validation).                 |
...
This is what powers dedup and freshness checks on the receiving end — see [Deduplication and Freshness](/docs/guides/websockets/advanced/deduplication-and-freshness).
