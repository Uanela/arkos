...
```ts
chatGateway.register(io, { store: myStore });
```
...
### Gotcha: the default store is single-instance only
...
```ts
chatGateway.register(io, { store: new RedisArkosGatewayStore(redis) });
```
