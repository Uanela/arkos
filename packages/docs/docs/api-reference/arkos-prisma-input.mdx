...
// ArkosPrismaService automatically handles the conversion
await userService.createOne(userData);
```

For more details on how Arkos handles relations, see [Handling Prisma Relation Fields](/docs/advanced-guide/handling-prisma-relations).
...
1. **No Runtime Transformation**: This is a type-only utility. The actual runtime transformation is handled by Arkos's relation handling system when using [ArkosPrismaService](/docs/api-reference/base-service) methods.

2. **Requires Arkos Services**: For the flattened format to work, you must use Arkos's ArkosPrismaService or custom services that extend it. Direct Prisma Client calls require traditional Prisma format.
...
- **[Handling Prisma Relation Fields](/docs/advanced-guide/handling-prisma-relations)** - How Arkos handles relations at runtime
- **[Interceptor Middlewares](/docs/core-concepts/interceptor-middlewares)** - Using with request interceptors
- **[ArkosPrismaService Class](/docs/api-reference/base-service)** - Service layer integration
