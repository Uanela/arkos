...
Service Hooks allow you to execute custom business logic at the service layer level during CRUD operations. Unlike [Interceptor Middlewares](/docs/guide/interceptor-middlewares) which run at the HTTP request level, Service Hooks execute whenever BaseService methods are called - whether through API endpoints or programmatic service calls.
