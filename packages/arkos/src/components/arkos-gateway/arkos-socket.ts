...
class ArkosBroadcastOperatorImpl {
  constructor(
    private readonly socket: ArkosSocket,
    private readonly operator: ArkosBroadcastOperator
  ) {
    return Object.assign(this.operator, this) as ArkosBroadcastOperator &
...
class ArkosRetryTargetImpl implements ArkosRetryTarget {
  ...
  /**
   * Emits with exponential backoff retry. `_meta` is injected automatically.
   * Retries on failure up to `maxRetries` times with delays: 1s, 2s, 4s... capped at 5s.
   *
   * @example
   * socket.retry(3).emit("notification", data)
   */
  emit(event: string, data: any, ...rest: any[]): boolean {
    const patched = injectMeta(data);
    return this.attemptEmit(event, patched, rest, 0);
  }
  ...
  private attemptEmit(
    ...
    try {
      ...
      if (attempt < this.maxRetries) {
        ...
        setTimeout(
          () => this.attemptEmit(event, data, rest, attempt + 1),
          delay
        );
        return false;
      }
...
export function mountArkosSocketExtensions(socket: ArkosSocket): void {
  ...
  /**
   * Emits an event to this client. `_meta` (`mid` + `timestamp`) is injected
   * automatically into every outgoing payload by Arkos.
   *
   * @example
   * socket.emit("message", { text: "hello" })
   */
  (socket as any).emit = function (event: string, data?: any, ...rest: any[]) {
   ...
   */
  (socket as any).emitWithAck = function (
    event: string,
    data?: any,
    ...rest: any[]
  ) {
   ...
   * Returns an `ArkosBroadcastOperator` with enhanced `.except({ user })` support
   ...
   */
  (socket as any).to = function (room: string | string[]) {
    return new ArkosBroadcastOperatorImpl(socket, originalTo(room as any));
  };

  /**
   * Sets a timeout in milliseconds for the next `emitWithAck`.
   * `_meta` is injected automatically on the underlying `emitWithAck` call.
   *
   * @example
   * await socket.timeout(5000).emitWithAck("confirm", data)
   */
  (socket as any).timeout = function (ms: number) {
    ...
    timedSocket.emitWithAck = function (
      ...
      return originalTimedEmitWithAck(event, injectMeta(data), ...rest);
    };
    return timedSocket;
  };
   ...
   * @since 1.7.0-canary.29
   */
  (socket as any).user = function (userId: string): ArkosUserTarget {
   ...
   * @since 1.7.0-canary.29
   */
  (socket as any).peer = function (socketId: string): ArkosSocket {
   ...
   * Delays between attempts: 1s, 2s, 4s... capped at 5s.
   * Chain `.timeout(ms)` before `.emitWithAck()` to set a per-attempt timeout.
   *
   * @example
   ...
   * @since 1.7.0-canary.29
   */
  (socket as any).retry = function (times: number): ArkosRetryTarget {
    return new ArkosRetryTargetImpl(socket, times);
  };
}
