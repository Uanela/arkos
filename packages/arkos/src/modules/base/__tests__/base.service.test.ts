...
describe("BaseService", () => {
  ...
  describe("createOne", () => {
    ...
    it("should execute beforeCreateOne and afterCreateOne hooks", async () => {
      ...
      expect(serviceHooksManager.handleHook).toHaveBeenCalledWith(
        mockAfterHook,
        expect.objectContaining({ result, data: body })
      );
    });
  });
  ...
  describe("findMany", () => {
    ...
    it("should execute beforeFindMany and afterFindMany hooks", async () => {
      ...
      expect(serviceHooksManager.handleHook).toHaveBeenCalledWith(
        mockAfterHook,
        expect.objectContaining({ result, filters })
      );
    });
  });

  describe("findOne", () => {
    it("should find a record by id using findUnique", async () => {
      const filters = { id: "1" };
      const expectedData = { id: "1", title: "Test Post" };
    ...
    it("should find a record by non id field using findFirst", async () => {
      const filters = { title: "Test Post" };
      const expectedData = { id: "1", title: "Test Post" };
  ...
  describe("updateOne", () => {
    ...
    it("should return null if record not found", async () => {
      mockPrisma.post.update.mockRejectedValue(
  ...
  describe("count", () => {
    it("should return the count when no filters are passed", async () => {
      const expectedCount = 7;
      mockPrisma.post.count.mockResolvedValue(expectedCount);
      const result = await baseService.count({});
  ...
  describe("BaseService - Additional Coverage", () => {
    let baseService: BaseService<any>;
    let mockPrisma: any;
    ...
    describe("count method edge cases", () => {
      it("should handle count with undefined filters", async () => {
        mockPrisma.post.count.mockResolvedValue(5);

        const result = await baseService.count(undefined);
      ...
      it("should execute count hooks when available", async () => {
        const mockBeforeHook = jest.fn();
        const mockAfterHook = jest.fn();
        ...
        await baseService.count({ published: true });

        expect(serviceHooksManager.handleHook).toHaveBeenCalledWith(
          mockBeforeHook,
          expect.objectContaining({ filters: { published: true } })
        );
        expect(serviceHooksManager.handleHook).toHaveBeenCalledWith(
          mockAfterHook,
          expect.objectContaining({ result: 3, filters: { published: true } })
        );
      });
    });

    describe("findOne edge cases", () => {
      it("should handle 'me' as id filter by using findFirst", async () => {
        const filters = { id: "me" };
        const expectedData = { id: "me", name: "Current User" };
      ...
      it("should execute findOne hooks when available", async () => {
        const mockBeforeHook = jest.fn();
        const mockAfterHook = jest.fn();
    ...
    describe("updateMany edge cases", () => {
      it("should execute updateMany hooks when available", async () => {
        const mockBeforeHook = jest.fn();
        const mockAfterHook = jest.fn();
        ...
        expect(serviceHooksManager.handleHook).toHaveBeenCalledWith(
          mockAfterHook,
          expect.objectContaining({ filters, data, result: { count: 2 } })
        );
      });

      it("should handle updateMany with query options", async () => {
        const filters = { published: false };
        const data = { published: true };
    ...
    describe("deleteOne edge cases", () => {
      it("should execute deleteOne hooks when available", async () => {
        const mockBeforeHook = jest.fn();
        const mockAfterHook = jest.fn();
    ...
    describe("deleteMany edge cases", () => {
      it("should execute deleteMany hooks when available", async () => {
        const mockBeforeHook = jest.fn();
        const mockAfterHook = jest.fn();
        ...
        const filters = { published: false };
        mockPrisma.post.deleteMany.mockResolvedValue({ count: 3 });
        ...
        expect(serviceHooksManager.handleHook).toHaveBeenCalledWith(
          mockAfterHook,
          expect.objectContaining({ filters, result: { count: 3 } })
        );
      });
    });

    describe("createMany edge cases", () => {
      it("should handle empty data array in createMany", async () => {
        mockPrisma.post.createMany.mockResolvedValue({ count: 0 });
      ...
      it("should execute createMany hooks when available", async () => {
        const mockBeforeHook = jest.fn();
        const mockAfterHook = jest.fn();
    ...
    describe("findById edge cases", () => {
      it("should execute findById hooks when available", async () => {
        const mockBeforeHook = jest.fn();
        const mockAfterHook = jest.fn();
        ...
        expect(serviceHooksManager.handleHook).toHaveBeenCalledWith(
          mockAfterHook,
          expect.objectContaining({ id, result: { id: "1", title: "Test" } })
        );
      });

      it("should handle findById with query options", async () => {
        const id = "1";
        const queryOptions = { select: { title: true } };

        mockPrisma.post.findUnique.mockResolvedValue({ title: "Test" });
    ...
    describe("Error handling", () => {
      ...
      it("should handle Prisma errors in update operations", async () => {
        mockPrisma.post.update.mockRejectedValue(new Error("Update failed"));

        await expect(
          baseService.updateOne({ id: "1" }, { title: "Test" })
        ).rejects.toThrow("Update failed");
      });
    });

    describe("Context propagation", () => {
      it("should propagate context to all hook methods", async () => {
        const mockContext = { user: { id: 1 } } as any;
        const mockHook = jest.fn();
        ...
        (getModuleComponents as jest.Mock).mockReturnValue({
          hooks: {
            beforeCreateOne: mockHook,
            afterCreateOne: mockHook,
            beforeFindMany: mockHook,
            afterFindMany: mockHook,
            beforeUpdateOne: mockHook,
            afterUpdateOne: mockHook,
            beforeDeleteOne: mockHook,
            afterDeleteOne: mockHook,
          },
        });

        mockPrisma.post.create.mockResolvedValue({ id: "1" });
        mockPrisma.post.findMany.mockResolvedValue([]);
        mockPrisma.post.update.mockResolvedValue({ id: "1" });
        mockPrisma.post.delete.mockResolvedValue({ id: "1" });

        // Test context propagation for various methods
        await baseService.createOne({ title: "Test" }, {}, mockContext);
        await baseService.findMany({}, {}, mockContext);
        await baseService.updateOne(
          { id: "1" },
          { title: "Test" },
          {},
          mockContext
        );
        await baseService.deleteOne({ id: "1" }, mockContext);

        // Verify context was passed to all hooks
        expect(serviceHooksManager.handleHook).toHaveBeenCalledWith(
          mockHook,
